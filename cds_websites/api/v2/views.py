...
class StudyPlansViewSet(mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def get_queryset(self):
        ...
        for q in query:
            schede = DidatticaPianoSche.objects.filter(regpiani_id__exact=q["regpiani_id"]).values(
                "sche_piano_id",
                "sche_piano_des",
                "sche_piano_cod",
                "pds_cod",
            ...
            for s in schede:
                obbl = DidatticaPianoSceltaSchePiano.objects.filter(sche_piano_id__exact=s["sche_piano_id"], tipo_sce_cod__exact="O").values(
                    "sce_id",
                    "sce_des",
                    "anno_corso",
                    "apt_slot_ord_num",
                ...
                for af in obbl:
                    ...
                    af_obblig = DidatticaPianoSceltaAf.objects.filter(sce_id__exact=af["sce_id"]).values(
                        "amb_id_af",
                        "anno_corso_af",
                        "ciclo_des",
                        "af_gen_des",
                    ...
                    for activity in af["required"]:
                        list_submodules = (
                            DidatticaAttivitaFormativa.objects.filter(part_stu_cod__isnull=True,af_pdr_id=activity["af_id"],)
                    ...
                    fil_and = DidatticaPianoSceltaFilAnd.objects.filter(sce_id__exact=af["sce_id"]).values(
                        "sce_fil_and_id",
                        "sce_id",
                        "sce_fil_or_id",
                        "sce_fil_or_des",
