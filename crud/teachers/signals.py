...
def pre_save_image(sender, instance, *args, **kwargs):
    ...
    try:
        old = instance.__class__.objects.get(pk=instance.pk).path_foto.path
        try:
...
def pre_save_cvita(sender, instance, *args, **kwargs):
    """ instance old image file will delete from os """
    try:
        old = instance.__class__.objects.get(pk=instance.pk).path_cv_ita.path
        try:
...
def pre_save_cven(sender, instance, *args, **kwargs):
    """ instance old image file will delete from os """
    try:
        old = instance.__class__.objects.get(pk=instance.pk).path_cv_en.path
        try:
