...
@extend_schema_serializer(examples=examples.BASE_RESEARCH_LINES_SERIALIZER_EXAMPLE)
class BaseResearchLinesSerializer(serializers.ModelSerializer):
    ...
    def get_teachers(self, obj):
        return [
            {
                "id": str(docente.personale.id_ab),
                "name": (
...
@extend_schema_serializer(examples=examples.APPLIED_RESEARCH_LINES_SERIALIZER_EXAMPLE)
class AppliedResearchLinessSerializer(serializers.ModelSerializer):
    description = serializers.CharField(source="descrizione")
    results = serializers.CharField(source="descr_pubblicaz_prog_brevetto")
    year = serializers.IntegerField(source="anno")
    aster2Id = serializers.CharField(
        source="ricerca_aster2_id.ricerca_aster1_id", allow_null=True
    )
    aster2Name = serializers.CharField(
        source="ricerca_aster2_id.descrizione", allow_null=True
    )
    teachers = serializers.SerializerMethodField()
    visible = serializers.BooleanField(source="visibile")
