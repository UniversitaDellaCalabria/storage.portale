...
from cds.models import DidatticaSsd
...
from ..settings import PHD_ADDITIONAL_REF_STRUCTURES, PHD_CYCLES
...
class DidatticaDottoratoAttivitaFormativaForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        structures = UnitaOrganizzativa.objects.filter(
            Q(uo_padre=STRUCTURES_FATHER) | Q(uo__in=PHD_ADDITIONAL_REF_STRUCTURES),
        ...
        lista_rif_dott = [
            ("Tutti i corsi di dottorato", "Tutti i corsi di dottorato"),
        ]
        ...
        self.fields["rif_dottorato"] = forms.ChoiceField(
            label=_("Reference"), choices=lista_rif_dott, required=True
        )
        self.fields["ciclo"] = forms.ChoiceField(
            label=_("Cycle"), choices=PHD_CYCLES, required=True
        )
