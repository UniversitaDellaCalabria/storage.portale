...
class BaseArticoliModelForm(forms.ModelForm):
    class Meta:
        ...
        labels = {"testo_it": _("Text (it)"), "testo_en": _("Text (en)")}
...
class DidatticaCdsArticoliRegolamentoNoteForm(forms.ModelForm):
    ...
    class Meta:
        ...
        help_texts = {
            "note": _(
                "This field is shared between the main article and its sub articles"
            )
        }


class DidatticaCdsArticoliRegolamentoTestataNoteForm(
...
class DidatticaCdsTestataStatusForm(forms.Form):
    motivazione = forms.CharField(
        label=_("Reason"), required=True, widget=forms.widgets.Textarea()
    )
...
class RegolamentoPdfImportForm(forms.Form):
    ...
    first_page = forms.ChoiceField(
        label=_(
            "Number of the page which contains 'TITOLO I' - Count from 1 from the title page (included)"
        ),
    ...
    last_page = forms.ChoiceField(
        label=_(
            "Number of the page which contains the last article untill the end - Count from 1 from the title page (included)"
        ),
        choices=(((num, num) for num in range(1, 200))),
    )
