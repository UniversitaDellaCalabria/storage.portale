...
FILETYPE_MEDIA = getattr(settings, "FILETYPE_MEDIA", app_settings.FILETYPE_MEDIA)
FILETYPE_ALLOWED = getattr(settings, "FILETYPE_ALLOWED", app_settings.FILETYPE_ALLOWED)
FILETYPE_PDF_ALLOWED = getattr(settings, "FILETYPE_PDF", app_settings.FILETYPE_PDF)
FILE_MAX_SIZE = getattr(settings, "FILE_MAX_SIZE", app_settings.FILE_MAX_SIZE)
FILETYPE_IMAGE_YX_RATIO_MIN = getattr(
    settings, "FILETYPE_IMAGE_YX_RATIO_MIN", app_settings.FILETYPE_IMAGE_YX_RATIO_MIN
)
FILETYPE_IMAGE_YX_RATIO_MAX = getattr(
    settings, "FILETYPE_IMAGE_YX_RATIO_MAX", app_settings.FILETYPE_IMAGE_YX_RATIO_MAX
)
FILETYPE_XLSX_ALLOWED = getattr(settings, "FILETYPE_XLSX", app_settings.FILETYPE_XLSX)
...
def orcid_validator(value):  # pragma: no cover
    ...
    if not re.match(regex, value):
        raise ValidationError(
            _(
                "Invalid ORCID. Remember, ORCID is an https URI with a 16-digit number that is compatible as 0000-0001-2345-6789"
            )
        )
...
def validate_image_size_ratio(value):  # pragma: no cover
    ...
    if mimetype in FILETYPE_IMAGE:
        ...
        ratio = y / w
        if ratio < FILETYPE_IMAGE_YX_RATIO_MIN or ratio > FILETYPE_IMAGE_YX_RATIO_MAX:
            rratio = f"{ratio:.2f}"
