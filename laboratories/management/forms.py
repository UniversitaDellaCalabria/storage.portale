...
class LaboratorioDatiBaseForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        if operator_departments_codes:
            department_choices = DidatticaDipartimento.objects.filter(
                dip_cod__in=operator_departments_codes
            ).values_list("dip_id", "dip_des_it")
        else:
            department_choices = DidatticaDipartimento.objects.all().values_list(
                "dip_id", "dip_des_it"
            )

        self.fields["choosen_department_id"] = forms.MultipleChoiceField(
            label=_("Related Department"),
    ...
    class Meta:
        ...
        fields = [
            "nome_laboratorio",
            "acronimo",
            "logo_laboratorio",  # 'laboratorio_interdipartimentale',
            "altre_strutture_riferimento",
            "descr_altre_strutture_riferimento_it",
        ...
        labels = {
            ...
            "descr_altre_strutture_riferimento_it": _(
                "Other Structures Description (it)"
            ),
...
class LaboratorioDatiBaseInterdipartimentaleForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        super(LaboratorioDatiBaseInterdipartimentaleForm, self).__init__(
            *args, **kwargs
        )
        ...
        self.fields["laboratorio_interdipartimentale"] = forms.ChoiceField(
            ...
            help_text=_(
                "Extra departments selection is available only after selecting 'YES' and then saving"
            ),
        )
...
class LaboratorioDatiBaseInfoSedeStruttureForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["sede_note_descrittive"].help_text = _(
            "Any addition to the provided data"
        )
        self.fields["strumentazione_descrizione"].help_text = _(
            "Public-use text (detail list below)"
        )
...
class LaboratorioDatiBaseStrumentazioneValoreForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        super(LaboratorioDatiBaseStrumentazioneValoreForm, self).__init__(
            *args, **kwargs
        )
        self.fields["strumentazione_valore"].validators = [
    ...
    class Meta:
        ...
        labels = {"strumentazione_valore": _("Equipment - Overall Value")}
...
class LaboratorioDatiBaseScientificDirectorChoosenPersonForm(forms.Form):
    choosen_scientific_director = forms.CharField(
        label=_("Scientific Director"), widget=forms.HiddenInput(), required=False
    )

    def __init__(self, *args, **kwargs):
        required = kwargs.pop("required", False)
        super(LaboratorioDatiBaseScientificDirectorChoosenPersonForm, self).__init__(
            *args, **kwargs
        )
        if required:
...
class LaboratorioDatiBaseSafetyManagerChoosenPersonForm(forms.Form):
    choosen_safety_manager = forms.CharField(
        label=_("Safety Manager"), widget=forms.HiddenInput(), required=False
    )

    def __init__(self, *args, **kwargs):
        required = kwargs.pop("required", False)
        super(LaboratorioDatiBaseSafetyManagerChoosenPersonForm, self).__init__(
            *args, **kwargs
        )
        if required:
...
class LaboratorioDatiBaseDipartimentoForm(forms.Form):
    choosen_department = forms.CharField(
        label=_("Department"), widget=forms.HiddenInput(), required=True
    )
...
class LaboratorioAttrezzatureForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["costo_unitario"] = forms.FloatField(
            label=_("Unit Cost"), min_value=0
        )
        self.fields["quantita"] = forms.IntegerField(label=_("Quantity"), min_value=1)

    class Meta:
        model = LaboratorioAttrezzature
        fields = ["tipologia", "descrizione", "costo_unitario", "quantita"]
        labels = {"tipologia": _("Type"), "descrizione": _("Description")}
...
class LaboratorioDatiErc1Form(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["erc0_selector"] = forms.ChoiceField(
            label=_("ERC Classification"), choices=erc0_choices
        )
...
class LaboratorioPersonaleForm(forms.Form):
    laboratory_staff = forms.CharField(label=_("Name and Surname"), max_length=200)


class LaboratorioPersonaleTecnicoForm(forms.Form):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["ruolo"] = forms.CharField(required=True, label=_("Role"))

        self.fields["percentuale_impegno"] = forms.FloatField(
            required=True, label=_("Commitment %"), min_value=0, max_value=100
        )


class LaboratorioAttivitaForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        activity_types_already_specified = kwargs.pop(
            "activity_types_already_specified", ()
        )
        super().__init__(*args, **kwargs)
        activity_types = LaboratorioTipologiaAttivita.objects.exclude(
            id__in=activity_types_already_specified
        ).values_list("id", "descrizione")

        self.fields["tipologia_attivita"] = forms.ChoiceField(
            label=_("Activity Type"), choices=activity_types, required=True
        )
...
class LaboratorioServiziErogatiForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["anno"] = forms.IntegerField(
            max_value=dt.today().year, min_value=1901, required=True, label=_("Year")
        )
        self.fields["importo_euro"] = forms.FloatField(
            min_value=0, required=True, label=_("Monetary Amount (Euros)")
        )
        self.fields["durata_mesi"] = forms.IntegerField(
            min_value=0, required=True, label=_("Duration (Months)")
        )
...
class LaboratorioServiziErogatiResponsabileForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["laboratory_manager"] = forms.CharField(
            required=True, label=_("Name and Surname")
        )
