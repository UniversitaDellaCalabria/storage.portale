...
from research_lines.models import RicercaErc1
...
from .forms import (
    LaboratorioAltriDipartimentiForm,
    LaboratorioAttivitaForm,
    LaboratorioAttrezzatureFondiForm,
    LaboratorioAttrezzatureForm,
    LaboratorioAttrezzatureRischiForm,
    LaboratorioDatiBaseAmbitoForm,
    LaboratorioDatiBaseForm,
    LaboratorioDatiBaseInfoSedeStruttureForm,
    LaboratorioDatiBaseInterdipartimentaleForm,
    LaboratorioDatiBaseSafetyManagerForm,
    LaboratorioDatiBaseScientificDirectorChoosenPersonForm,
    LaboratorioDatiBaseScientificDirectorForm,
    LaboratorioDatiBaseStrumentazioneValoreForm,
    LaboratorioDatiErc1Form,
    LaboratorioPersonaleForm,
    LaboratorioPersonaleTecnicoForm,
    LaboratorioServiziErogatiForm,
    LaboratorioServiziErogatiResponsabileForm,
    LaboratorioServiziOffertiForm,
    LaboratorioTipologiaRischioForm,
    LaboratorioUbicazioneForm,
)
from .utils import _get_user_roles, _is_user_scientific_director
...
def laboratory_info_sede_edit(
    ...
    if request.POST:
        laboratoriodatibaseinfosedestruttureform = (
            LaboratorioDatiBaseInfoSedeStruttureForm(
                data=request.POST, instance=laboratory
            )
        )
        if laboratoriodatibaseinfosedestruttureform.is_valid():
            ...
            request.session["lab_infosedestrutture_form_data"] = (
                laboratoriodatibaseinfosedestruttureform.data
            )

    return redirect(
        reverse("laboratories:management:laboratory-edit", kwargs={"code": code})
...
@can_manage_laboratories
@can_view_laboratories
def laboratory_equipment_value_edit(
    request, code, laboratory=None, my_offices=None, is_validator=False
):
    if not (
        request.user.is_superuser
    ...
    if request.POST:
        ...
        if laboratoriodatibasestrumentazionevaloreform.is_valid():
            ...
            request.session["lab_strumentazionevalore_form_data"] = (
                laboratoriodatibasestrumentazionevaloreform.data
            )
...
@can_manage_laboratories
@can_view_laboratories
def laboratory_interdepartmental_edit(
    request, code, laboratory=None, my_offices=None, is_validator=False
):
    if not (
        request.user.is_superuser
    ...
    if request.POST:
        ...
        if laboratoriodatibaseinterdipartimentaleform.is_valid():
            ...
            request.session["lab_interdipartimentale_form_data"] = (
                laboratoriodatibaseinterdipartimentaleform.data
            )
...
def laboratory(request, code, laboratory=None, my_offices=None, is_validator=False):
    ...
    lab_infosedestrutture_form_data = request.session.pop(
        "lab_infosedestrutture_form_data", None
    )
    lab_ambito_form_data = request.session.pop("lab_ambito_form_data", None)
    lab_strumentazionevalore_form_data = request.session.pop(
        "lab_strumentazionevalore_form_data", None
    )
    lab_interdipartimentale_form_data = request.session.pop(
        "lab_interdipartimentale_form_data", None
    )

    # LaboratorioDatiBase
    user_lab_offices = OrganizationalStructureOfficeEmployee.objects.filter(
    ...
    allowed_related_departments_codes = (
        []
        if (not user_lab_offices.exists())
        else user_lab_offices.values_list(
    ...
    laboratoriodatibaseinfosedestruttureform = LaboratorioDatiBaseInfoSedeStruttureForm(
        instance=laboratory, data=lab_infosedestrutture_form_data
    )
    laboratoriodatibaseambitoform = LaboratorioDatiBaseAmbitoForm(
        instance=laboratory, data=lab_ambito_form_data
    )
    laboratoriodatibasestrumentazionevaloreform = (
        LaboratorioDatiBaseStrumentazioneValoreForm(
            instance=laboratory, data=lab_strumentazionevalore_form_data
        )
    )
    laboratoriodatibaseinterdipartimentaleform = (
        LaboratorioDatiBaseInterdipartimentaleForm(
            instance=laboratory, data=lab_interdipartimentale_form_data
        )
    )
    # LaboratorioTipologiaRischio
    ...
    logs = LogEntry.objects.filter(
        content_type_id=ContentType.objects.get_for_model(laboratory).pk,
        object_id=laboratory.pk,
    )

    breadcrumbs = {
        reverse("generics:dashboard"): _("Dashboard"),
        reverse("laboratories:management:laboratories"): _("Laboratory"),
    ...
    return render(
        ...
        "laboratory.html",
        {
            ...
            "user_roles": _get_user_roles(
                request, laboratory, my_offices, is_validator
            ),
        },
    )
...
def laboratory_new(request, laboratory=None, my_offices=None, is_validator=False):
    ...
    allowed_related_departments_codes = (
        []
        if (not my_offices or not my_offices.exists())
        else my_offices.values_list(
            "office_id__organizational_structure_id__unique_code", flat=True
        )
    )
    ...
    scientific_director_internal_form = (
        LaboratorioDatiBaseScientificDirectorChoosenPersonForm(required=True)
    )
    ...
    if request.POST:
        ...
        if (
            "choosen_scientific_director" in request.POST
            and request.POST["choosen_scientific_director"]
        ):
            scientific_director_form = (
                LaboratorioDatiBaseScientificDirectorChoosenPersonForm(
                    data=request.POST, required=True
                )
            )
        else:
            scientific_director_form = LaboratorioDatiBaseScientificDirectorForm(
                data=request.POST
            )
        ...
        if form.is_valid() and scientific_director_form.is_valid():
            ...
            if scientific_director_form.cleaned_data.get("choosen_scientific_director"):
                ...
                laboratory.responsabile_scientifico = (
                    f"{scientific_director.cognome} {scientific_director.nome}"
                )
                ...
                laboratory.responsabile_scientifico = (
                    scientific_director_form.cleaned_data["responsabile_scientifico"]
                )

                laboratory.matricola_preposto_sicurezza = None
                laboratory.preposto_sicurezza = scientific_director_form.cleaned_data[
            ...
            log_action(
                user=request.user, obj=laboratory, flag=ADDITION, msg=[{"added": {}}]
            )
            ...
            messages.add_message(
                ...
                messages.SUCCESS,
                _(
                    "Laboratory anagraphic created successfully, please fill in the other fields"
                ),
            )
            return redirect(
...
@can_view_laboratories
@can_edit_laboratories
def laboratory_scientific_director_edit(
    request, code, laboratory=None, my_offices=None, is_validator=False
):
    scientific_director = None
    ...
    if request.POST:
        ...
        if form.is_valid():
            if form.cleaned_data.get("choosen_person"):
                ...
                laboratory.matricola_responsabile_scientifico = None
                laboratory.responsabile_scientifico = form.cleaned_data[
                    "responsabile_scientifico"
                ]
    ...
    breadcrumbs = {
        ...
        reverse("laboratories:management:laboratories"): _("Laboratories"),
        reverse(
            "laboratories:management:laboratory-edit", kwargs={"code": code}
        ): laboratory.nome_laboratorio,
        reverse(
...
@can_view_laboratories
@can_edit_laboratories
def laboratory_safety_manager_edit(
    request, code, laboratory=None, my_offices=None, is_validator=False
):
    safety_manager = None
    ...
    if request.POST:
        ...
        if form.is_valid():
            if form.cleaned_data.get("choosen_person"):
                ...
                laboratory.preposto_sicurezza = (
                    f"{safety_manager.cognome} {safety_manager.nome}"
                )
...
@can_view_laboratories
@can_edit_laboratories
def laboratory_safety_manager_delete(
    request, code, laboratory=None, my_offices=None, is_validator=False
):
    laboratory.matricola_preposto_sicurezza = None
    ...
    log_action(
        user=request.user, obj=laboratory, flag=CHANGE, msg=_("Deleted safety manager")
    )
...
@check_if_superuser
@can_manage_laboratories
def laboratory_delete(
    request, code, laboratory=None, my_offices=None, is_validator=False
):
    laboratory.delete()
    messages.add_message(
        request, messages.SUCCESS, _("Laboratory removed successfully")
    )

    return redirect("laboratories:management:laboratories")
        ...
        if department_form.is_valid() and department_form.cleaned_data.get(
            ...
            if department.dip_id == laboratory.id_dipartimento_riferimento_id:
                messages.add_message(
                    ...
                    messages.ERROR,
                    _(
                        "Extra departments must be different from Laboratory's department"
                    ),
                )
            else:
...
def laboratory_equipment_new(
    ...
    if request.POST:
        ...
        if (
            ...
            and equipment_funds_form.is_valid()
    ...
    breadcrumbs = {
        ...
        reverse(
            "laboratories:management:laboratory-equipment-new", kwargs={"code": code}
        ): _("Equipment"),
    }
    return render(
        request,
        "laboratory_unique_form.html",
        {
            ...
            "forms": (
                equipment_form,
                equipment_funds_form,
                equipment_risks_form,
            ),
            "laboratory": laboratory,
...
def laboratory_equipment_edit(
    ...
    if request.POST:
        ...
        equipment_form = LaboratorioAttrezzatureForm(
            instance=laboratory_equipment, data=request.POST
        )
        equipment_funds_form = LaboratorioAttrezzatureFondiForm(data=request.POST)
        equipment_risks_form = LaboratorioAttrezzatureRischiForm(data=request.POST)
...
@can_manage_laboratories
@can_view_laboratories
def laboratory_researches_erc1_edit(
    request, code, laboratory=None, my_offices=None, is_validator=False
):
    ...
    if request.POST:
        ...
        if research_erc1_form.is_valid():
            ...
            selected_erc1_res_ids = research_erc1_form.cleaned_data.get(
            ...
            current_erc1_res = list(
                set(selected_erc1_res_ids) - set(current_erc1_res_ids)
            )
...
def laboratory_locations_edit(
    request, data_id, code, laboratory=None, my_offices=None, is_validator=False
):
...
def laboratory_locations_new(
    ...
    breadcrumbs = {
        ...
        reverse(
            "laboratories:management:laboratory-locations-new", kwargs={"code": code}
        ): _("Locations"),
    }
    return render(
        request,
...
def laboratory_locations_delete(
    ...
    log_action(
        user=request.user, obj=laboratory, flag=CHANGE, msg=_("Deleted location")
    )
...
@can_view_laboratories
@can_edit_laboratories
def laboratory_research_staff_new(
    request, code, laboratory=None, my_offices=None, is_validator=False
):
    researcher = None
...
def laboratory_research_staff_delete(
    ...
    log_action(
        user=request.user, obj=laboratory, flag=CHANGE, msg=_("Deleted researcher")
    )
...
@can_view_laboratories
@can_edit_laboratories
def laboratory_technical_staff_new(
    request, code, laboratory=None, my_offices=None, is_validator=False
):
    technician = None
    internal_form = ChoosenPersonForm(required=True)
        ...
        if person_form.is_valid() and technician_form.is_valid():
            if person_form.cleaned_data.get("choosen_person"):
                ...
                LaboratorioPersonaleTecnico.objects.create(
                    ...
                    percentuale_impegno=technician_form.cleaned_data.get(
                        "percentuale_impegno"
                    ),
                )
            else:
...
def laboratory_technical_staff_delete(
    ...
    log_action(
        user=request.user, obj=laboratory, flag=CHANGE, msg=_("Deleted technician")
    )
...
def laboratory_activities_new(
    ...
    if request.POST:
        ...
        if activity_form.is_valid():
            ...
            log_action(
                user=request.user, obj=laboratory, flag=CHANGE, msg=_("Added activity")
            )
    ...
    breadcrumbs = {
        ...
        reverse(
            "laboratories:management:laboratory-activities-new", kwargs={"code": code}
        ): _("Activities"),
    }
    return render(
        request,
...
def laboratory_activities_edit(
    ...
    activity_form = LaboratorioAttivitaForm(
        activity_types_already_specified=activity_types_already_specified,
        instance=activity,
        initial={"tipologia_attivita": selected_activity_id},
    )
    ...
    if request.POST:
        ...
        if activity_form.is_valid():
            ...
            log_action(
                user=request.user, obj=laboratory, flag=CHANGE, msg=_("Edited activity")
            )
...
def laboratory_activities_delete(
    ...
    log_action(
        user=request.user, obj=laboratory, flag=CHANGE, msg=_("Deleted activity")
    )
...
@can_view_laboratories
@can_edit_laboratories
def laboratory_provided_services_new(
    request, code, laboratory=None, my_offices=None, is_validator=False
):
        ...
        if person_form.is_valid() and form.is_valid():
            ...
            if person_form.cleaned_data.get("choosen_person"):
                ...
                provided_service.responsabile_origine = (
                    f"{manager.cognome} {manager.nome}"
                )
            else:
                provided_service.matricola_responsabile = None
                provided_service.responsabile_origine = person_form.cleaned_data[
                    "laboratory_manager"
                ]
...
def laboratory_provided_services_edit(
    ...
    if request.POST:
        ...
        form = LaboratorioServiziErogatiForm(
            data=request.POST, instance=provided_service
        )

        if "choosen_person" in request.POST and request.POST["choosen_person"]:
            person_form = ChoosenPersonForm(
                initial=cpf_initial, data=request.POST, required=False
            )
        else:
            person_form = LaboratorioServiziErogatiResponsabileForm(
                initial=lpf_initial, data=request.POST
            )
...
@can_view_laboratories
@can_edit_laboratories
def laboratory_offered_services_new(
    request, code, laboratory=None, my_offices=None, is_validator=False
):
...
def laboratory_offered_services_edit(
    ...
    if request.POST:
        ...
        form = LaboratorioServiziOffertiForm(
            data=request.POST, instance=offered_service
        )
...
def laboratory_offered_services_delete(
    ...
    log_action(
        user=request.user, obj=laboratory, flag=CHANGE, msg=_("Deleted offered service")
    )
...
@can_view_laboratories
@can_edit_laboratories
def laboratory_risk_types_edit(
    request, code, laboratory=None, my_offices=None, is_validator=False
):
    if request.POST:
        ...
        if risk_type_form.is_valid():
            # LaboratorioTipologiaRischio
            new_selected_risks = risk_type_form.cleaned_data.get(
                "tipologie_rischio", []
            )
            ...
            messages.add_message(
                request, messages.SUCCESS, _("Risk types updated successfully")
            )
        else:  # pragma: no cover
...
@can_view_laboratories
@can_edit_laboratories
def laboratory_request_approval(
    request, code, laboratory=None, my_offices=None, is_validator=False
):
    ...
    log_action(
        user=request.user, obj=laboratory, flag=CHANGE, msg=_("Requested approval")
    )
...
def laboratory_approve(
    ...
    log_action(
        user=request.user, obj=laboratory, flag=CHANGE, msg=_("Approved Laboratory")
    )
