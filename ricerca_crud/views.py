...
def patent(request, code,
    ...
    if request.POST:
        form = BrevettoDatiBaseForm(instance=patent, data=request.POST)
        if form.is_valid():
...
def patent_inventor(request, code, inventor_id, inventors,
    ...
    if inventor:
        ...
        form = DidatticaCdsAltriDatiCoordinatorForm(initial={'choosen_person': inventor_data[0]})
    else:
        form = DidatticaCdsAltriDatiCoordinatorForm()
        ...
        if request.POST:
            ...
            if form.is_valid():
                ...
                log_action(user=request.user,
                           obj=patent,
                           flag=CHANGE,
                           msg=f'{_("Added inventor")} {inventor.__str__()}')
