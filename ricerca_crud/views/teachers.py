    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            other_data = DocentePtaAltriDati.objects.create(
                path_foto=form.cleaned_data['path_foto'] if form.cleaned_data.get(
                    'path_foto') else None,
                matricola=teacher,
                path_cv_ita=form.cleaned_data['path_cv_ita'] if form.cleaned_data.get(
                    'path_cv_ita') else None,
                path_cv_en=form.cleaned_data['path_cv_en'] if form.cleaned_data.get(
                    'path_cv_en') else None,
...
def teacher_materials_data_edit(request, code, data_id,
    ...
    if request.POST:
        form = DocenteMaterialeDidatticoForm(
            instance=materials, data=request.POST)
        if form.is_valid():
