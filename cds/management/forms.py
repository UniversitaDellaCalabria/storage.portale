...
class DidatticaCdsGruppoForm(forms.ModelForm):
    class Meta:
        ...
        # }
        widgets = {
            "descr_breve_it": forms.Textarea(attrs={"rows": 2}),
...
class DidatticaCdsGruppoComponenteForm(forms.ModelForm):
    class Meta:
        ...
        fields = ["visibile", "nome", "cognome", "funzione_it", "funzione_en", "ordine"]
...
class DidatticaRegolamentoAltriDatiForm(forms.ModelForm):
    def __init__(
        self, clob_type, clob_label, tipo_testo_regdid_cod, form_name, *args, **kwargs
    ):
        ...
        if clob_type == "PDF":  # pdf file
            self.fields["clob_txt_ita"] = forms.FileField(
                validators=[validate_pdf_file_extension, validate_file_size]
            )
            self.fields["clob_txt_eng"] = forms.FileField(
                validators=[validate_pdf_file_extension, validate_file_size]
            )
            ...
            self.fields["clob_txt_ita"] = forms.CharField(
                widget=forms.widgets.Textarea()
            )
            self.fields["clob_txt_eng"] = forms.CharField(
                widget=forms.widgets.Textarea()
            )
        # common
        ...
        self.fields["form_name"] = forms.CharField(
            initial=form_name, widget=forms.widgets.HiddenInput()
        )
        # tipo_testo_regdid_cod
        self.fields["tipo_testo_regdid_cod"] = forms.CharField(
            initial=tipo_testo_regdid_cod, widget=forms.widgets.HiddenInput()
        )
